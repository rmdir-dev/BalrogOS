...
/*  Shared heap code.
    ...
    This file is compiled once and linked into the kernel and into the libc.
    kheap.c, vmheap.c and malloc.c only own the boundaries of their heap and
    the way it grows, every block manipulation is done here. If the allocator
    is buggy, this is the only file to fix.

    A block is always sizeof(block_info) + _size bytes long, the blocks tile
    the heap from its start to its top and the free blocks are chained by
    ascending address through next_free, the last one pointing to the top.
*/
    ...
    */
    if(current_block->_size >= size)
    {
        ...
        if(current_block->_size <= size + sizeof(block_info))
        {
            ...
            if(first_block && current_block == (block_info*)*first_free)
            {
...
void heap_free(block_info* block, block_info* next_block, block_info* current_top, uintptr_t* first_free, uint64_t block_max_size)
{
    ...
    while(1)
    {
        if(next_block < current_top)
        {
            ...
            if(!next_block->_is_mmapped)
            {
                ...
                if(block->_size + sizeof(block_info) < block_max_size && contiguous)
                {
                    ...
                    // the absorbed block is not a block anymore.
                    // it can't stay the first free block.
                    if((block_info*)*first_free == next_block)
                    {
                        *first_free = (uintptr_t)new_free_block;
                    }
                } else
                {
        ...
        {
            // the next free block is the heap top.
            block_info* free_block = block;
