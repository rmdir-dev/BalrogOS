...
int __vfs_device_read(fs_device_t* device, uint8_t* buffer, uint64_t len, fs_fd* fd)
{
    ...
    for (size_t i = 0; i < fd->vfs_node->children_size; i++)
    {
        ...
        entry->inbr = -1;
        entry->entry_size = EXT2_DIR_ENTRY_SIZE(strlen(child->name));
        entry->type = EXT2_TYPE_BLOCK_DEVICE;
        entry->name_len = strlen(child->name);
        memcpy(&entry->name, child->name, entry->name_len);
